...
function Page() {
  ...
  const [questionSet, setQuestionSet] = useState([]); // New state to store the questions and options
              ...
              {createAnOption && (
                <div className="m-2 flex flex-row gap-2">
                  <input
                    ...
                    className="h-8 border-2 border-cyan-800 rounded"
                  />
                  <button
      ...
      <div className="font-bold text-xl mb-4">Preview</div>
              ...
              <div className="flex flex-col gap-2">
                {options.map((option, index) => (
                  <div className="flex flex-row">
                    ...
                    <div
                      key={index}
                      className="flex shadow-md flex-col m-1 p-2 w-80 rounded bg-cyan-300 text-cyan-800"
                    >
                      {option}
                    </div>
                  </div>
        ...
        {questionSet.length > 0 && (
          <div className="mt-8 m-2">
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  {question.options.map((option, optIndex) => (
                    <div className="flex flex-row">
                        <div className="bg-slate-500 text-white flex justify-center items-center h-6 w-6 my-2" style={{borderRadius:'50%'}}>
                            {optIndex + 1}
                        </div>
                    <div key={optIndex} className="flex flex-row flex-wrap h-10 rounded p-2 ml-4 w-80 bg-white shadow-md">
                      {option}
                    </div>
                    </div>
                  ))}
            ...
            <button className="w-full bg-pink-600 mx-2 hover:bg-pink-500 rounded text-white p-3">Publish Question Paper</button>
          </div>
        )}
